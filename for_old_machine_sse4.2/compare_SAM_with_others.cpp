	...
	**/
	...
	while(!in.eof()){
		...
		if(id_string[0] != '@')
		{
			...
			if(first_i != -1)
			{
				tmp = o_err.substr(first_i + 1);

				istringstream istr200(tmp);
				istr200 >> o_err_int; 
			}
			...
			///in >> strand >> chr >> pos;
			in >> strand >> chr >> pos >> tmp >> tmp >> tmp >> tmp >> tmp >> read_sequence >> read_qual >> err;
			...
			if(first_i != -1)
			{
				tmp = err.substr(first_i + 1);

				istringstream istr300(tmp);
				istr300 >> err_int; 
			}
			...
			**/
			...
			if(o_chr == chr && (labs(o_position_p - pos) <= 50))
			{
			...
			else
			{
				...
				**/
				if (o_err_int < err_int)
				{
					cerr<<"id: "<<id_string<<endl;
					cerr<<"o_chr: "<<o_chr<<endl;
					cerr<<"o_position_p: "<<o_position_p<<endl;
					cerr<<"o_err: "<<o_err<<endl;
					cerr<<"o_err_int: "<<o_err_int<<endl;
